---
type: index
title: AI-Native Engineering
description: "Building software when AI agents are first-class participants in the SDLC — adoption, agent operation, and context engineering."
tags: [meta, ai-engineering, agentic-workflows]
topic: engineering/ai-native
status: complete
updated: 2026-07-05
---

# AI-Native Engineering

Notes on building software when AI agents are first-class participants in the SDLC:
the agentic SDLC, adoption strategy, team topologies, agent operation, context
engineering, and the pipeline that keeps agent-generated change safe.

Start with the **[Overview](ai-native-engineering-overview.md)** — it's the thematic map
for this section. Classic delivery practices (testing, CI/CD, release engineering) live
in **[Practices](../practices/)**.

## Notes

### Overview & maps
- [AI-Native Engineering — Overview](ai-native-engineering-overview.md)
- [The AI-Native Engineering Stack](ai-native-engineering-stack.md)
- [AI in the SDLC — Terminology](ai-sdlc-terminology.md)

### The agentic SDLC & methodologies
- [The Agentic SDLC](agentic-sdlc.md)
- [AI-DLC and the Reimagined SDLC](ai-dlc-methodology.md)
- [Spec-Driven Development](spec-driven-development.md)
- [Loop-Driven Development](loop-driven-development.md)
- [Eval-Driven Development for AI Capabilities](eval-driven-ai-development.md)
- [Agile in the Age of AI](agile-in-the-age-of-ai.md)

### Strategy, adoption & maturity
- [Agentic AI Strategy Frameworks](agentic-ai-strategy-frameworks.md)
- [Agentic SDLC Maturity Model](agentic-sdlc-maturity-model.md)
- [Scaling AI Adoption in the SDLC](scaling-ai-adoption.md)
- [The AI Factory](ai-factory.md)
- [The APEX Framework — Measuring AI Engineering Impact](apex-framework.md)
- [Modern Engineering Values](modern-engineering-values.md)
- [AI Demands More Engineering Discipline](ai-engineering-discipline.md)

### Teams & organisation
- [The AI-Augmented Engineering Team](ai-augmented-engineering-team.md)
- [Team Topologies for the Agentic Platform](team-topologies-agentic-platform.md)
- [The Trust Factory](trust-factory.md)

### Agents in practice
- [Agentic Autonomy Levels](agentic-autonomy-levels.md)
- [Long-Running Agents](long-running-agents.md)
- [Backpressure Loops for Coding Agents](agent-backpressure-loops.md)
- [Own the Outer Loop](own-the-outer-loop.md)
- [Agentic Code Review](agentic-code-review.md)
- [Harness Engineering](harness-engineering.md)
- [Quality-First AI Coding](quality-first-ai-coding.md)

### Context & prompting
- [Prompt Engineering for Programmers](prompt-engineering-for-programmers.md)
- [CRESS Principles for Context Engineering](cress-context-engineering.md)

### Pipeline & infrastructure
- [CI/CD as the Control Plane for AI-Assisted Engineering](ci-cd-ai-engineering.md)
- [CI Speed When Agents Write the Code](ci-speed-with-ai-agents.md)
- [Model Routing and AI Gateways](model-routing-and-ai-gateways.md)
