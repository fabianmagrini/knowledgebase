...
- [Latency Patterns](latency-patterns.md) — a taxonomy of nineteen latency techniques in four categories (locality, work reduction, concurrent execution, anticipation), the trade each one makes, and the argument that AI applications lose most of their time outside the model call — "time to first useful result" rather than completion
